...
# operations based on those values. To access arguments within a Bash script,
# use special variables. The arguments are passed to a script as follows.

./my_script.sh arg1 arg2 arg3

# Where `my_script.sh` is the name of the script and `arg1`, `arg2`, `arg3`, etc., are the arguments to be
# passed to the script. Below are some special Bash variables used to
# access the arguments.

: 1. `$0`: Represents the name of the script itself.

: 2. `$1`, `$2`, `$3`, ...: Represent the first, second, third argument and so on. For example, `$1`
: represents `arg1`, `$2` represents `arg2`, and so on.

: 3. `$#`: Represents the total number of arguments passed to the script.

: 4. `$*` or `$@`: Represents all arguments passed to the script as a single text string. However,
: they behave slightly differently. `$*` treats all arguments as a single string, while `$@` treats them as separate arguments. In general, `$@` is used more frequently.

: 5. `$?`: Represents the exit code (exit status) of the previous command. It can be useful to check if
: the previous command was executed correctly.
